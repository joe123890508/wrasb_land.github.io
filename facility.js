...
async function loadAllData() {
  ...
  try {
    ...
    vardata = await res1.json(); // 指派給頂層宣告的 vardata
    renderGeoJson(vardata);
  } catch (error) {
    console.error("資料載入失敗", error);
    alert("資料載入失敗，請稍後再試！");
  } finally {  if (loadingOverlay) loadingOverlay.classList.add('d-none'); }
}
...
function renderGeoJson(data) {
  ...
  for (let i = 0; i < data.features.length; i++) {
    let unit = data.features[i].properties.保管單位;
...
document.getElementById("confirm").addEventListener("click", () => {
  if (!vardata) return; // 確保資料載入完成才執行過濾
  ...
  document.querySelectorAll("button.search-button").forEach(btn => {
    btn.addEventListener("click", () => { map.flyTo([btn.getAttribute("lat"), btn.getAttribute("lng")], 18); });
  });
  if (results.length === 0) { tbody.innerHTML = `<tr><td colspan="4" class="text-center text-muted">沒有符合的資料</td></tr>`; }
});
